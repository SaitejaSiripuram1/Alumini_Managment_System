...
class AlumniUploadView(FormView):
    ...
    # Log the uploaded file
        print("Uploading file:", self.request.FILES["file"])
    
    # Call handle_alumni_csv and check if the CSV is valid
        status = handle_alumni_csv(self.request.FILES["file"])
    
    # If the status is False, raise a validation error
        if not status:
            raise ValidationError("Invalid file structure!")
    
        print(f"CSV validation status: {status}")  # Log the status of the CSV validation
